...
should_ignore_mmcblk0_partition() {
    local dev="$1"
    ...
    if command -v fw_printenv >/dev/null 2>&1; then
        bootslot=$(fw_printenv bootslot 2>/dev/null | cut -d '=' -f2)
    else
        logger "mount.sh: fw_printenv not found, cannot check bootslot"
        return 1
    fi
    ...
    case "$bootslot" in
        dualA)
            if [ "$part_num" = "3" ] || [ "$part_num" = "4" ]; then
                logger "mount.sh: ignoring $dev because bootslot=dualA and partition $part_num should not be mounted"
                return 0
            fi
            ;;
        dualB)
            if [ "$part_num" = "1" ] || [ "$part_num" = "2" ]; then
                logger "mount.sh: ignoring $dev because bootslot=dualB and partition $part_num should not be mounted"
                return 0
            fi
            ;;
        *)
...
if [ "$ACTION" = "add" ] && [ -n "$DEVNAME" ] && [ -n "$ID_FS_TYPE" -o "$media_type" = "cdrom" ]; then
    if should_ignore_mmcblk0_partition "$DEVNAME"; then
        exit 0
    fi
