...
void cf_sha256_init(void *ctx)
{
	cf_sha256_context *ref_ctx = (cf_sha256_context *)ctx;
	memset(ctx, 0, sizeof *ref_ctx);
	ref_ctx->H[0] = 0x6a09e667;
	ref_ctx->H[1] = 0xbb67ae85;
	ref_ctx->H[2] = 0x3c6ef372;
	ref_ctx->H[3] = 0xa54ff53a;
	ref_ctx->H[4] = 0x510e527f;
	ref_ctx->H[5] = 0x9b05688c;
	ref_ctx->H[6] = 0x1f83d9ab;
	ref_ctx->H[7] = 0x5be0cd19;
}

void cf_sha224_init(void *ctx)
{
	cf_sha256_context *ref_ctx = (cf_sha256_context *)ctx;
	memset(ref_ctx, 0, sizeof *ref_ctx);
	ref_ctx->H[0] = 0xc1059ed8;
	ref_ctx->H[1] = 0x367cd507;
	ref_ctx->H[2] = 0x3070dd17;
	ref_ctx->H[3] = 0xf70e5939;
	ref_ctx->H[4] = 0xffc00b31;
	ref_ctx->H[5] = 0x68581511;
	ref_ctx->H[6] = 0x64f98fa7;
	ref_ctx->H[7] = 0xbefa4fa4;
}
...
void cf_sha256_update(void *ctx, const void *data, size_t nbytes)
{
	cf_sha256_context *ref_ctx = (cf_sha256_context *)ctx;
	cf_blockwise_accumulate(ref_ctx->partial, &ref_ctx->npartial,
				sizeof ref_ctx->partial, data, nbytes,
				sha256_update_block, ref_ctx);
}

void cf_sha224_update(void *ctx, const void *data, size_t nbytes)
{
	cf_sha256_update(ctx, data, nbytes);
}

void cf_sha256_digest(void *ctx, uint8_t hash[CF_SHA256_HASHSZ])
{
	...
	cf_sha256_context ours = *(cf_sha256_context *)ctx;
...
void cf_sha224_digest(void *ctx, uint8_t hash[CF_SHA224_HASHSZ])
{
	uint8_t full[CF_SHA256_HASHSZ];
	cf_sha256_context *ref_ctx = (cf_sha256_context *)ctx;
	cf_sha256_digest(ref_ctx, full);
	memcpy(hash, full, CF_SHA224_HASHSZ);
}

void cf_sha224_digest_final(void *ctx, uint8_t hash[CF_SHA224_HASHSZ])
{
	uint8_t full[CF_SHA256_HASHSZ];
	cf_sha256_context *ref_ctx = (cf_sha256_context *)ctx;
	cf_sha256_digest_final(ref_ctx, full);
	memcpy(hash, full, CF_SHA224_HASHSZ);
}
