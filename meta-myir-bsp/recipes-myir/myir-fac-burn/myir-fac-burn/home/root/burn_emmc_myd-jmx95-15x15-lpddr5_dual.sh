...
########################################
# Partition layout: A/B dual-rootfs
#
#   p1: BOOT_A   (vfat, kernel+dtb)
#   p2: ROOTFS_A (ext4)
#   p3: BOOT_B   (vfat, kernel+dtb)
#   p4: ROOTFS_B (ext4)
########################################
mksdcard() {
...
burn_bootloader() {
    ...
    echo 0 > /sys/block/mmcblk${PART}boot1/force_ro
    dd if=${UBOOT_FILE} of=/dev/mmcblk${PART}boot0
    dd if=${UBOOT_FILE} of=/dev/mmcblk${PART}boot1
    cmd_check $? "burn uboot failed"
    sync
    echo 1 > /sys/block/mmcblk${PART}boot0/force_ro
    echo 1 > /sys/block/mmcblk${PART}boot1/force_ro
}
