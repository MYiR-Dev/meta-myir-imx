...
echo -e "\n
    Supported machines: `echo; ls sources/meta-freescale/conf/machine/*.conf \
	                          sources/meta-myir/meta-myir-bsp/conf/machine/*.conf \
        |egrep "/(imx6|imx7|imx8|imx9).*conf$" | sed s/\.conf//g | sed -r 's/^.+\///' | xargs -I% echo -e "\t%"`


    Supported NXP's i.MX distros: `echo; ls sources/meta-myir/meta-myir-sdk/conf/distro/fsl-*.conf \
...
echo "BBLAYERS += \"\${BSPDIR}/sources/meta-hailo\"" >> $BUILD_DIR/conf/bblayers.conf
