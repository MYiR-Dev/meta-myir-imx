...
#-----------------------------------------------------------------------------
#
# File: hsm_hab4_pki_tree.sh
#
# Description: This script generates a basic HAB4 PKI tree for the Freescale
#              HAB code signing feature using HSM. What the PKI tree looks like
#              depends on whether the SRK is chosen to be a CA key or not.  If
#              the SRKs are chosen to be CA keys then this script will generate
#              the following PKI tree:
#
#                                      CA Key
#                                      | | |
#                             -------- + | +---------------
...
printf "    script is available for this.  This script assumes openssl\n"
...
usage()
{
    ...
    echo "$0 -existing-ca <y/n> [-ca-key-label <CA key label> -ca-cert <CA cert name>] -use-ecc <y/n> -kl <ECC/RSA Key Length> -duration <years> -num-srk <1-4> -srk-ca <y/n>"
    echo "Options:"
...
do
    ...
    if [ $use_ecc = 'n' ]
        then
            srk_key_i_label=SRK${i}_sha256_${kl}_65537_v3_ca
            srk_crt_i=../crts/SRK${i}_sha256_${kl}_65537_v3_ca_crt

            csf_key_label=CSF${i}_1_sha256_${kl}_65537_v3_usr
            csf_subj_req=/CN=CSF${i}_1_sha256_${kl}_65537_v3_usr/
            csf_crt=../crts/CSF${i}_1_sha256_${kl}_65537_v3_usr_crt
            csf_key_type=rsa:${kl}
        else
            srk_key_i_label=SRK${i}_sha256_${cn}_v3_ca
            srk_crt_i=../crts/SRK${i}_sha256_${cn}_v3_ca_crt

            csf_key_label=CSF${i}_1_sha256_${cn}_v3_usr
